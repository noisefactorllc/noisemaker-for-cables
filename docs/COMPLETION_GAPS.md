...
- Status: open. Priority: P2. Category: authority. Pre-delivery rejection implemented and the declaration narrowed at this commit; closure still requires the exact-source kit export verification (served kit evidence).
...
- Implemented behavior: `export-kit/kit.config.json` now declares `compat.mode: "list"` with the explicit 209 effect IDs that the recorded rendered parity qualifies (`parity/coverage-matrix.json` denominator: 210 catalog ids, 209 rendered and compared at zero mismatched channels including the three GAP-007 overlay-settle effects, `filter/octaveWarp` compile-only and unqualified). `tools/compat-gate.mjs` derives that scope from the committed evidence and rejects any declaration claiming more (mode `all`, an unqualified id, an omitted qualified id, or a duplicate) before delivery; `test/compat-gate.test.js` covers the positive run and negative identity controls (8 tests, part of `npm test`, 265 pass / 0 fail at this source).
- Authority limits: the scaffold export-kit builder and its consuming host remain outside this repository, so their own rejection tests and enforcement cannot be executed from here; this record closes only after the served kit's `compat.json` is verified to equal the committed declaration at the published source SHA.
- Evidence: `export-kit/kit.config.json`, `tools/compat-gate.mjs`, `test/compat-gate.test.js`, `authority-comparison.json`, and GAP-001/GAP-002/GAP-007 recorded qualification.
- Next action: publish, fetch the served kit at the published source (`deployment-meta.json` git_hash) and verify `compat.json` byte-matches the committed declaration; then close this record.
...
- Required checks: builder rejection tests (implemented as the in-repo gate and its negative controls; scaffold-side builder tests remain an authority limit) and exact-source kit export.
- Last verification: 2026-09-26 (declaration narrowed and gate tested at this commit; served kit not yet re-verified).
