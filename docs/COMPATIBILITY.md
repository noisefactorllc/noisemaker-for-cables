...
| 2026-09-26 | this record commit | Kit declaration narrowed. Served-kit verification pending. | GAP-006 in-repo part: `export-kit/kit.config.json` declares `compat.mode: "list"` with the explicit 209 effect IDs the recorded parity qualifies (210 catalog ids minus the compile-only `filter/octaveWarp`; the three overlay effects are settled at zero mismatched channels under GAP-007). `tools/compat-gate.mjs` + `test/compat-gate.test.js` reject any declaration claiming more before delivery (unit suite 265 pass, 0 fail). The served kit's `compat.json` must be verified against this declaration at the published source SHA before GAP-006 closes. |
